...
import {DataElementInformationForSpreadsheet, SpreadsheetColNames} from '../src/elements/ProfileElement';
...
describe('ProfileElement', () => {
    ...
    describe('#elemName', () => {
        ...
        test('should indicate slice in element name', async () => {
            const elem = await mcode_stu1.getElement('CancerGenomicsReport',
        ...
        test('should not include slice parents called "Extension"', async () => {
            const elem = await mcode_stu1.getElement('CancerRelatedRadiationProcedure',
    ...
    describe('#required', () => {
        test('should indicate conditional for nested requires', async () => {
            const elem = await mcode_stu1.getElement('CancerPatient',
    ...
    describe('#valueSet', () => {
        test('should get value set info for a CodeableConcept', async () => {
            const elem = await mcode_stu1.getElement('GeneticSpecimen',
                'Specimen.type'
            );
            expect(elem.elem.valueSet.uri).toBe(
        ...
        test('should get value set info for a code', async () => {
            const elem = await mcode_stu1.getElement('GenomicRegionStudied',
                'Observation.status'
            );
    ...
    describe('#elemType', () => {
        test('should include profile name in type for references', async () => {
            const elem = await mcode_stu2_with_dependencies.getElement('mcode-genomics-report',
    ...
    describe('#definition', () => {
        test('should include a custom description', async () => {
            const elem = await mcode_stu2.getElement('ComorbiditiesElixhauser',
                'Observation.focus'
            );
            expect(elem.elem.definition).toContain('A reference to the cancer condition that is the context for the current list of comorbid conditions');

        });
    });

    test('should include category with fixed value slice in "all" mode', async () => {
        const elem = await mcode_stu1.getElement('mcode-ecog-performance-status',
            'Observation.category', false
        );
    ...
    test('should include fixed value slice with suppressedFixedCodes setting != true', async () => {
        ...
        expect(j[0][SpreadsheetColNames.Required]).toContain('Required [Fixed to http://loinc.org#89247-1]');
    });

    test('should handle slicing pattern in mCODE STU1 for setting value[x] type', async () => {
        const elem = await mcode_stu1.getElement('mcode-cancer-disease-status',
    ...
    test('in MS mode, should suppress sliced element row in favor of displaying slices when all slices are MS', async () => {
        let elem = await mcode_stu2.getElement(
            'CancerRelatedMedicationAdministration', 'MedicationAdministration.extension',
            true
        );
        ...
        elem = await mcode_stu2.getElement(
            'CancerRelatedMedicationAdministration', 'MedicationAdministration.extension:treatmentIntent',
            true
        );
        expect(elem.elem.toJSON().length).toBe(1);
        elem = await mcode_stu2.getElement(
            'CancerRelatedMedicationAdministration', 'MedicationAdministration.extension:terminationReason',
            true
        );
        expect(elem.elem.toJSON().length).toBe(1);
    ...
    test.skip('should not show `:someSlice` in favor of `:someSlice.value[x]`', async () => {
        const elem1 = await mcode_stu1.getElement('mcode-cancer-genetic-variant',
            'Observation.component:GenomicDNAChange', true);
        const elem2 = await mcode_stu1.getElement('mcode-cancer-genetic-variant',
            'Observation.component:GenomicDNAChange.value[x]', true);
    ...
    describe('Polymorphic elements', () => {
       test('MS effective[x] and MS effective[x]:effectiveDateTime', async () => {
           const effective = await mcode_stu3.getElement('mcode-genomic-variant', 'Observation.effective[x]');
           const effectiveDateTime = await mcode_stu3.getElement('mcode-genomic-variant', 'Observation.effective[x]:effectiveDateTime');
           ...
           expect(effectiveDateTime.elem.elem.id).toBe('Observation.effective[x]:effectiveDateTime');
